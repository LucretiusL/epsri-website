...
(function () {
  ...
  async function loadData(symbol) {
    ...
    if (endpoint) {
      try {
        ...
        setStatus(`代理连接失败，已回退：${error.message}`, 'warning');
      }
    }
  ...
  function updateMetrics() {
    ...
    $('dataModeLabel').textContent = data.mode === 'LIVE_PROXY' ? 'LIVE PROXY CONNECTED' : 'SIMULATED + ADAPTER READY';
    setStatus(data.mode === 'LIVE_PROXY' ? '已接入外部 JSON 代理数据' : '研究级示例数据，支持接入 JSON 代理端点', data.mode === 'LIVE_PROXY' ? 'ok' : 'warning');
